...
class Stitch(object):
    ...
    POST_TWITTER = False
    POST_TO_FB = True
    def get_photo_batch(self, batch_id):
    ...
    def stitch_photos(self, batch_id):
        ...
        matching = self.get_photo_batch(batch_id)
        ...
        if self.SAVE_HORIZONTAL:
            self.make_horizontal(min_shape, imgs)
        vertical_image_path = self.make_vertical(min_shape, imgs)
        shark_image_path = self.add_shark_border(vertical_image_path)
    ...
    def make_vertical(self, min_shape, imgs):
        ...
        vertical_image_name = '{0}/Vertical-{1}.jpg'.format(
            self.VERTICAL_PHOTOS_DIR, int(time.time()))
